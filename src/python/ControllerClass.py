...
import time
import numpy as np
from src.python.JoystickActions import Actions


class Controller(Actions):

    def __init__(
            self, joystick_q,interface, connecting_using_ds4drv=True,
        ...
        Actions.__init__(self,joystick_q)
        self.stop = False
    ...
    def listen(self, joystick_q=None,timeout=30, on_connect=None, on_disconnect=None, on_sequence=None):
        """
    ...
    def __handle_event(self, button_id, button_type, value, overflow, debug):
        ...
        if event.R3_event():
            ...
            """
            I added the below 2 if statements 2 better capture right stick inputs
            """
            if event.button_id == 3: #X axis
                self.right_stick[0] = event.value
                self.get_right_stick_xy()
            elif event.button_id == 4: #Y axis
                self.right_stick[1] = event.value
                self.get_right_stick_xy()
            ...
            """
            I added the below 2 if statements 2 better capture left stick inputs
            """
            if event.button_id == 0: #X axis
                self.left_stick[0] = event.value
                self.get_left_stick_xy()
            elif event.button_id == 1: #Y axis
                self.left_stick[1] = event.value
                self.get_left_stick_xy()
        ...
        elif event.playstation_button_pressed():  #adding exit option on button press
            self.event_history.append("ps")
            self.on_playstation_button_press(exit)  #<----
