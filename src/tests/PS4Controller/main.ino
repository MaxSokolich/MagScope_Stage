...
void loop()
{
   ...
   Bx = (action[0] + Bx_roll) / sqrt(action[0]*action[0] + Bx_roll*Bx_roll);
   By = (action[1] + By_roll) / sqrt(action[1]*action[1] + By_roll*By_roll);
   Bz = (action[2] + Bz_roll) / sqrt(action[2]*action[2] + Bz_roll*Bz_roll) ;
   ...
   //Zero = actions =[0,0,0,alpha, gamma, 0]
